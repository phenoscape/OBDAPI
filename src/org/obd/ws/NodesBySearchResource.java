...
/**
 * Resource for multiple nodes based on search results
 */
public class NodesBySearchResource extends NodeResource {
    ...
    public Collection<Node> findNodes() throws Exception {
        ...
        if (searchTerm != null) { 
        	...
        	for (String s : searchTerms) {
        		if (op == null)
        			nodes.addAll(getShard().getNodesBySearch(s));
        		else
        			nodes.addAll(getShard().getNodesBySearch(s, op));
        	}
        }
    ...
    public Representation getRepresentation(Variant variant) {
    	...
    	if (format.equals("json")) {
    		...
    		sb.append("</table>\n");
           	sb.append(hrefToOtherFormats("/search/"+operatorString+"/"+searchTerm));
