...
/**
 * As {@link QueryTest}, but actually executing queries
 * @author cjm
 *
 */
public class ExecuteQueryTest extends QueryTest {

	public ExecuteQueryTest(String n) {
		super(n);
	...
	public Graph runLinkQuery(QueryTerm qt) throws SQLException, ClassNotFoundException {
		...
		assertTrue(stmts.size() > 0); // TODO - restore all these asserts
	...
	public Graph runLinkQuery(QueryTerm qt, Statement matchStmt) throws SQLException, ClassNotFoundException {
		...
		for (Statement s : stmts) {
			System.out.println(" "+s);
			if (matchStmt == null || s.toString().equals(matchStmt.toString()) ) {
				matchFound = true;
			}
		}
	...
	public static void addTests(TestSuite suite) {
		suite.addTest(new ExecuteQueryTest("testQuery"));
	}
