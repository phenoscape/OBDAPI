...
CREATE OR REPLACE VIEW instance_link_class_pair AS
 ...
 FROM
  ...
  INNER JOIN link ON (io.node_id=link.no1de_id)
...
UPDATE link SET reiflink_node_id=get_node_id('BIRN:generic_annotation') 
 WHERE predicate_id IN (SELECT node_id FROM is_bearer_of_relation_node) AND is_inferred='f' AND node_id IN (SELECT node_id FROM is_a_human) AND object_id NOT IN (SELECT node_id FROM is_disease_class);
...
-- conservative
-- e.g. Dopamine that part_of neuron; axon terminal in neostriatum
SELECT 
...
-- the following are less necessary now we are precoordinating these up-front

-- does not get degenerate in (axon terminal that part_of Neostr)
SELECT 
...
-- gets degenerate in (axon terminal that part_of Neostr)
SELECT 
   store_genus_differentium(node_type_id,get_node_id('OBO_REL:inheres_in'),object_type_id,get_node_id('BIRN:generic_annotation'))
...
-- todo: restrict. currently includes q^inh(x^p(y))^inhp(y)
SELECT 
   store_genus_differentium(node_id,get_node_id('OBO_REL:inheres_in_part_of'),object_id,get_node_id('BIRN:generic_annotation'))
 FROM inheres_in_part_of AS ip
 WHERE NOT EXISTS (SELECT * FROM obo_rel.inheres_in AS i WHERE i.node_id=ip.node_id AND i.object_id=ip.object_id);
