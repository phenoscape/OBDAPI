...
FROM 
  ...
  is_a_link,
  node inheres_in, 
  link inheres_in_link
WHERE
  is_a_link.is_inferred = false AND
  inheres_in_link.is_inferred = false AND
  phenotype.node_id = inheres_in_link.node_id AND
  phenotype.node_id = is_a_link.node_id AND
  inheres_in.node_id = inheres_in_link.predicate_id AND
  inheres_in.uid = 'OBO_REL:inheres_in'
  ;
