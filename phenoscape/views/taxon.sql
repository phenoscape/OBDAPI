...
SELECT DISTINCT
  ...
  has_rank_link.object_id AS rank,
  tagval.val AS is_extinct,
  parent_link.object_id AS parent
FROM
  node taxon
  ...
  LEFT OUTER JOIN link has_rank_link ON (has_rank_link.node_id = taxon.node_id AND has_rank_link.predicate_id = has_rank_rel.node_id)
